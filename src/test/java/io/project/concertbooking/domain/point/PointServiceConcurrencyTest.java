...
class PointServiceConcurrencyTest extends IntegrationTestSupport {
    ...
    class ChargeTest {
        ...
        @DisplayName("10번의 포인트 충전 요청이 동시에 오더라도 1번만 반영되어야 한다")
        ...
        void charge() throws Exception {
            ...
            for (int i = 0; i < concurrentRequestCount; i++) {
                executorService.submit(() -> {
                    try {
                        pointService.chargeWithDistributedSimpleLock(user, 1000);
    ...
    class UseTest {
        ...
        @DisplayName("10번의 포인트 사용 요청이 동시에 오더라도 1번만 반영되어야 한다")
        @RepeatedTest(value = 10, name = "{currentRepetition}/{totalRepetitions} - {displayName}")
        void use() throws Exception {
            ...
            for (int i = 0; i < concurrentRequestCount; i++) {
                executorService.submit(() -> {
                    try {
                        pointService.useWithDistributedSimpleLock(user, 1000);
                        successCount.incrementAndGet();
                    } catch (CustomException e) {
                        failCount.incrementAndGet();
                    } finally {
            ...
            Point point = pointOpt.get();
            assertThat(point.getAmount()).isEqualTo(9000);
        }
    }
}