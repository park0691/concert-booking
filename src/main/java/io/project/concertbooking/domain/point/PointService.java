...
public class PointService {
    ...
    @Transactional
    @DistributedSpinLock(key = "'point:' + #user.userId")
    public Point chargeWithDistributedSpinLock(User user, Integer chargePoint) {
        Optional<Point> pointOpt = pointRepository.findByUser(user);
    ...
    @Transactional
    @DistributedSpinLock(key = "'point:' + #user.userId")
    public Point useWithDistributedSpinLock(User user, Integer usePoint) {
        Point point = pointRepository.findByUser(user)
                .orElseGet(() -> pointRepository.savePoint(Point.createPoint(user, 0)));
